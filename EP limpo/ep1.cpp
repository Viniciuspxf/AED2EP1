...
template <class tipoNo, class Chave, class Item>
Item devolveR(tipoNo *no, Chave chave) {
    int comparacao;
    if (no != nullptr) {
        ...
        if (comparacao == 0) 
            ...
            return devolveR<tipoNo, Chave, Item>(no->esq, chave);
        else 
            return devolveR<tipoNo, Chave, Item>(no->dir, chave);
    }
    return -1;
}

template <class tipoNo, class Chave, class Item>
Chave selecionaR(tipoNo *no, int k, int *contador) {
    Chave chave;
    if (no == nullptr) {
        return "";
    }
    else {
        chave = selecionaR<tipoNo, Chave, Item>(no->esq, k, contador);
        if (*contador == k) return chave;
        *contador += 1;
        if (k == *contador) return no->chave;
        return selecionaR<tipoNo, Chave, Item>(no->dir, k, contador);
    }
}

template <class tipoNo>
void excluiArvore(tipoNo *no) {
    if (no != nullptr) {
        excluiArvore(no->dir);
        excluiArvore(no->esq);
        delete [] no;
...
template<class noTipo>
int contaNos(noTipo *no) {
    int contador = 0;
    if (no == nullptr) {
        return -1;
    }
    else {
        contador = contaNos<noTipo>(no->esq) + 1;
        contador += contaNos<noTipo>(no->dir) + 1;
    }
    return contador;
}

template <class noTipo, class Chave>
int posicao (noTipo *no, Chave chave) {
    ...
    while (no->pai != nullptr) {
        if (no->pai->dir == no){        
            contador++;
            contador += contaNos<noTipo>(no->pai->esq) + 1;
        }
        no = no->pai;
    }
    return contador;
...
arvoreBin<Chave, Item>::~arvoreBin() {
    excluiArvore<noArvore<Chave, Item>>(raiz);
}

template <class Chave, class Item>
void arvoreBin<Chave, Item>::printa(){
    printaR<noArvore<Chave, Item>>(raiz);
}
...
Item arvoreBin<Chave, Item>::devolve(Chave chave) {
    return devolveR<noArvore<Chave, Item>, Chave, Item>(raiz, chave);
}

template <class Chave, class Item>
int arvoreBin<Chave, Item>::rank(Chave chave) {
    return posicao<noArvore<Chave, Item>, Chave>(raiz, chave);
}
...
Chave arvoreBin<Chave, Item>::seleciona(int k) {
    int contador = -1;
    return selecionaR<noArvore<Chave, Item>, Chave, Item>(raiz, k, &contador);
}
...
treap<Chave, Item>::~treap(){
    excluiArvore<noTreap<Chave, Item>>(raiz);
}

template <class Chave, class Item>
void treap<Chave, Item>::printa(){
    printaR<noTreap<Chave, Item>>(raiz);
}
...
Item treap<Chave, Item>::devolve(Chave chave){
    return devolveR<noTreap<Chave, Item>, Chave, Item>(raiz, chave);
}
...
int treap<Chave, Item>::rank(Chave chave) {
    return posicao<noTreap<Chave, Item>>(raiz, chave);
}
...
Chave treap<Chave, Item>::seleciona(int k){
    int contador = -1;
    return selecionaR<noTreap<Chave, Item>, Chave, Item>(raiz, k, &contador);
}
...
arvoreRN<Chave, Item>::~arvoreRN() {
    excluiArvore<noRN<Chave, Item>>(raiz);
}

template <class Chave, class Item>
void arvoreRN<Chave, Item>::printa() {
    printaR<noRN<Chave, Item>>(raiz);
}
...
Item arvoreRN<Chave, Item>::devolve(Chave chave) {
    return devolveR<noRN<Chave, Item>, Chave, Item>(raiz, chave);
}
...
int arvoreRN<Chave, Item>::rank(Chave chave) {
    return posicao<noRN<Chave, Item>>(raiz, chave);
}

template <class Chave, class Item>
Chave arvoreRN<Chave, Item>::seleciona(int k) {
    int contador = -1;
    return selecionaR<noRN<Chave, Item>, Chave, Item>(raiz, k, &contador);
}
